...
| `impeccable` | [pbakaus/impeccable](https://github.com/pbakaus/impeccable) (Apache 2.0) | Qualità del design di interfacce: `audit`, `critique`, `polish`, `bolder`, `animate`, `adapt` e altri comandi. |
...
`.claude/skills/brainstorming/LICENSE.superpowers`. `impeccable` è la build per
Claude Code del commit `e0881d2`, con licenza e NOTICE nella sua cartella (senza
gli hook, che l'installer ufficiale mette in `.claude/settings.local.json`).

`CLAUDE.md` impone l'ordine per il web development: prima l'intervista di
`brainstorming`, poi, solo se servono, `hallmark`, `impeccable` e il connettore
Originkit per la parte visiva.
